...
app.get('/problem/:id', async (req, res) => {
  try {
    ...
    if (problem.is_public || problem.allowedEdit) {
      await syzoj.utils.markdown(problem, ['background', 'description', 'input_format', 'output_format', 'example', 'limit_and_hint']);
    } else {
      throw new ErrorMessage('您没有权限进行此操作。');
    }
...
app.post('/problem/:id/edit', async (req, res) => {
  try {
    ...
    problem.background = req.body.background;
...
app.post('/problem/:id/import', async (req, res) => {
  try {
    ...
    problem.background = json.obj.background;
...
app.post('/problem/:id/manage', app.multer.fields([{ name: 'testdata', maxCount: 1 }, { name: 'additional_file', maxCount: 1 }]), async (req, res) => {
  try {
	...
	problem.vjudge_config = req.body.vjudge_config;
    problem.vjudge_stdin = req.body.vjudge_stdin;
