...
app.post('/api/ide', async (req, res) => {
  try {
    ...
    const LANGUAGE_MAP = {
      'cpp':        105, 'cpp14': 54, 'c': 103,
      'python3':    113, 'python2': 70, 'java': 91,
      'javascript': 102, 'typescript': 101, 'rust': 108,
      'go':         107, 'kotlin': 111, 'scala': 112,
      'csharp':     51,  'php': 98,   'ruby': 72,
      'swift':      83,  'bash': 46,  'lua': 64,
      'pascal':     67,  'haskell': 61, 'perl': 85,
      'dart':       90,  'ocaml': 65, 'r': 99
    };
 
    if (!LANGUAGE_MAP[language]) {
      return res.json({ success: false, error: '不支持该语言' });
    }
    ...
    if (response.compile_output) {
      return res.json({ success: true, result: 'Compile Error', output: response.compile_output });
    }
 
    let result;
    switch (response.status.id) {
      case 3:  result = 'Accepted'; break;
      case 5:  result = 'Time Limit Exceeded'; break;
      case 6:  result = 'Memory Limit Exceeded'; break;
      case 11: result = 'Runtime Error (NZEC)'; break;
      default:
        result = response.status.id >= 7 ? 'Runtime Error' : 'Accepted';
    }
